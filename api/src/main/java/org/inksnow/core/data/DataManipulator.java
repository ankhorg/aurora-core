...
import org.inksnow.core.data.value.CopyableValueContainer;
...
public interface DataManipulator extends CopyableValueContainer {
     ...
     * {@link Value.Immutable}, and as such,
    ...
    interface Mutable extends DataManipulator {
        ...
        default Mutable copyFrom(final ValueContainer valueContainer, final MergeFunction overlap, final Key<?> first, final Key<?>... more) {
            final List<Key<?>> keys = Stream.concat(Stream.of(first), Arrays.stream(more)).collect(Collectors.toList());
            return this.copyFrom(valueContainer, overlap, Collections.unmodifiableList(keys));
        }
